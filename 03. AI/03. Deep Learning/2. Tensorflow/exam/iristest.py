...
data = pd.read_csv('./Iris2.csv', index_col = 0)
...
with graph.as_default():
    ...
    # loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits_2, tf_train_labels))
    softmax_y = tf.nn.softmax(logits_2)
    cross_entropy = tf.reduce_mean(-tf.reduce_sum(tf.log(softmax_y), reduction_indices=[1]))
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        _, l, predictions = session.run([optimizer, cross_entropy, predict_train])
